...
class TrainedLSTMAgent:
    ...
    def step(self, state):
        # 1. Extract the three components needed by the LSTM model
        obs = torch.FloatTensor(state['obs']).unsqueeze(0)  # Snapshot [1, 159]
        history = torch.FloatTensor(state['history']).unsqueeze(0)  # History [1, 15, 60]
        legal_actions = state['legal_actions']

        best_action, max_q = None, -float('inf')

        for action, feature in legal_actions.items():
            feat = torch.FloatTensor(feature).unsqueeze(0)  # Action [1, 60]

            with torch.no_grad():
                # Pass all three to the model: (z, x_state, x_action)
